...
data SequenceDictJSON = SequenceDictJSON
  { jWaveforms    :: !(Map Text [Float])
  , jWeights      :: !(Map Text [Float])
  , jAcquisitions :: !(Map Text Object)
  , jProgram      :: ![[Text]]
  }

instance FromJSON SequenceDictJSON where
  parseJSON = withObject "SequenceDict" $ \o -> do
    SequenceDictJSON
      <$> o .: "waveforms"
      <*> o .: "weights"
      <*> o .: "acquisitions"
...
loadSequenceDict :: BL.ByteString -> Either SimError SequenceDict
loadSequenceDict bs = case eitherDecode bs of
  Left e  -> Left (LoaderError e)
  Right j -> toSequenceDict j

toSequenceDict :: SequenceDictJSON -> Either SimError SequenceDict
toSequenceDict j = do
  let wfMap  = Map.map V.fromList (jWaveforms j)
      wtMap  = Map.map V.fromList (jWeights j)
      acqMap = Map.map (\_ -> AcqConfig 100) (jAcquisitions j)
  prog <- parseProgramTuples (jProgram j)
  return SequenceDict
    { sdWaveforms    = wfMap
    , sdWeights      = wtMap
    , sdAcquisitions = acqMap
    , sdProgram      = prog
    }

--------------------------------------------------------------------------------
-- * Direct tuple parsing (no text parser needed)
--------------------------------------------------------------------------------

type RawTuple = (Maybe Text, Text, [Text])

parseProgramTuples :: [[Text]] -> Either SimError [Instruction]
parseProgramTuples tuples = do
  rawTuples <- mapM extractTuple tuples
  let labels = buildLabelMap rawTuples
  mapM (resolveTuple labels) (zip [0..] rawTuples)

extractTuple :: [Text] -> Either SimError RawTuple
extractTuple [lbl, mnem, argsStr, _comment] =
  let args = filter (not . T.null) (map T.strip (T.splitOn "," argsStr))
      lbl' = if T.null lbl then Nothing else Just lbl
  in Right (lbl', mnem, args)
extractTuple xs =
  Left (LoaderError ("invalid program tuple: " ++ show xs))

buildLabelMap :: [RawTuple] -> Map Text Int
buildLabelMap = snd . foldl go (0, Map.empty)
  where
    go (idx, m) (Just lbl, _, _) = (idx + 1, Map.insert lbl idx m)
    go (idx, m) (Nothing, _, _)  = (idx + 1, m)

resolveTuple :: Map Text Int -> (Int, RawTuple) -> Either SimError Instruction
resolveTuple labels (idx, (_, mnem, args)) =
  case parseInstr idx labels (T.toLower mnem) args of
